...
<?php endwhile;?>
<?php get_template_part( 'parts/page-bording-central' ); ?>
<div class="full-width Bordas_download">
  <?php $query = new WP_Query( 'order=asc&category_name=portaler&post_status=publish&paged='. get_query_var('paged')); ?>
  <?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
  <div class="row" data-equalizer="bar">
    <div class="small-12 medium-6 large-6 columns" data-equalizer-watch="bar">
      <?php the_content(); ?>
    </div>
    <div class="small-12 medium-6 large-6 columns brochear" style="background-size: contain;background-image: url(<?php echo wp_get_attachment_url( get_post_thumbnail_id($post->ID) ); ?>)" data-equalizer-watch="bar">&nbsp;</div>
  </div>
  <?php endwhile;  wp_reset_postdata(); else : ?>
  <?php _e( 'Sorry, no posts matched your criteria.' ); ?>
  <?php endif; ?>
</div>
<?php $query = new WP_Query( 'order=asc&tag=apoteksvarer&post_status=publish&paged='. get_query_var('paged')); ?>
<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
...
<?php $query = new WP_Query( array('order' => 'asc', 'post_type' => 'kontakt-partners', 'kontakt_partner_catagory' => 'kontakt-vores-eksperter', 'post_status' => 'publish', 'posts_per_page' => -1)); ?>
    <?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
    ...
    <?php _e( 'Sorry, no posts matched your criteria.' ); ?>
    <?php endif; ?>
    <?php if(get_field('bottom_text')){  echo ' <div class="small-12 medium-12 large-12 columns left">' . get_field('bottom_text') . '</div>';}?>
  </div>
</div>